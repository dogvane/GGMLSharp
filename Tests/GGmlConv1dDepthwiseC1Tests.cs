...
{
    public sealed class GGmlConv1dDepthwiseC1Tests
    {
        public void Test_Conv1dDepthwise_C1_BackendGraph_Matches_UpstreamCpp()
        {
            ...
            try
            {
                ...
                {
                    throw new Exception("ggml_gallocr_reserve failed for conv1d-dw-c1 graph.");
                }
                ...
                {
                    throw new Exception("ggml_gallocr_alloc_graph failed for conv1d-dw-c1 graph.");
                }
                ...
                TestAssertions.Shape(output, 6, 2, 1, 1);
                TestAssertions.FloatDataNear(output, 1e-4f,
                    50.0f, 60.0f, 60.0f, 60.0f, 60.0f, 30.0f,
                    0.50f, 0.60f, 0.60f, 0.60f, 0.60f, 0.30f);
            }
            finally
            {
        ...
        private static SafeGGmlGraph BuildGraph(SafeGGmlContext context, SafeGGmlTensor weight, SafeGGmlTensor input)
        {
            ...
            var output = context.ConvDepthwise1D(weight, input, 1, 1, 1);
